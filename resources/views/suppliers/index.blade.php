<x-app-layout>
    <div class="content">
        ...
        <div class="bg-white rounded p-4">
            <h4 class="text-lg font-bold mb-5">{{ isset($supplier) ? 'Edit' : 'Add' }} Supplier
                <bdi>(فراہم کنندہ کا اندراج کریں)</bdi>
            </h4>
...
{{--                                step="0.01"--}}
{{--                                class="mt-1 block w-full"--}}
{{--                                :value="old('discount',$supplier->discount ?? 0)"--}}
{{--                                required--}}
{{--                            />--}}
                    ...
                    <div>
                        <div>
                            ...
                            <label class="relative inline-flex items-center cursor-pointer mt-1">
                                ...
                                <input name="status" type="checkbox" value="1" @if(old('status', $supplier->status ?? 1) == 1) checked @endif class="sr-only peer">
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="details"
                                      name="details"
                                      class="w-full mt-1"
                                      rows="4">{{old('details',$supplier->details ?? '')}}</textarea>
                            <x-input-error class="mt-2" :messages="$errors->get('details')"/>
                        </div>
                    </div>
                </div>
                <div class="text-right mt-4">
                    <x-primary-button type="submit">{{ isset($supplier) ? 'Update' : 'Add' }}</x-primary-button>
                </div>
            </form>
        </div>
