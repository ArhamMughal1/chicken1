<x-app-layout>
    <div class="content" x-data="saleEntry">
        <div class="mb-3">
            <h1 class="text-primary-100 text-2xl font-bold">Edit Client Sales
                <bdi>(دکاندارون کاکھاتہ تدوین)</bdi>
            </h1>
            @if(!isset($today_rate))
                <div class="text-right text-sm bg-red-100 border border-red-400 text-red-700 px-3 py-2 rounded relative mt-2" role="alert">
        ...
        <div class="bg-white rounded p-4">
            ...
            <div class="mb-4 flex justify-between items-center">
                <h4 class="text-lg font-bold">Edit Sale
            ...
            <!-- The form should point to the update route -->
            <form action="{{ route('sale.batch-update', $date) }}" method="POST" dir="rtl">
                @csrf
                @method('PUT')
                ...
                <!-- Client rows with pre-filled data -->
                <template x-for="(client, index) in filteredClients" :key="client.id">
                    <div class="grid grid-cols-1 gap-1 md:grid-cols-9 mt-2"
                        ...
                        <div>
                            <input type="hidden" x-bind:name="'clients[' + index + '][client_id]'" x-bind:value="client.id">
                            <x-text-input
                                type="text"
                                class="mt-1 block w-full font-nastaliq !leading-nastaliq"
                                x-bind:value="client.full_name"
                                readonly
                            />
                        ...
                        <div>
                            <x-text-input
                                dir="ltr"
                                x-bind:name="'clients[' + index + '][amount]'"
                                type="number"
                                step="0.01"
                                class="mt-1 block w-full text-right !leading-nastaliq"
                                x-model="amount"
                        ...
                        <div>
                            <x-text-input
                                dir="ltr"
                                x-bind:name="'clients[' + index + '][amount_paid]'"
                                type="number"
                                class="mt-1 block w-full text-right !leading-nastaliq"
                                x-model="amountPaid"
                                @input="updateCalculations"
                        ...
                        <div>
                            <x-text-input
                                dir="ltr"
                                x-bind:name="'clients[' + index + '][arrears]'"
                                type="number"
                                class="mt-1 block w-full text-right !leading-nastaliq"
                                x-model="arrears"
                                readonly
                        ...
                        <div>
                            <x-text-input
                                dir="ltr"
                                x-bind:name="'clients[' + index + '][previous_arrears]'"
                                type="number"
                                class="mt-1 block w-full text-right !leading-nastaliq"
                        ...
                        <div>
                            <x-text-input
                                dir="ltr"
                                x-bind:name="'clients[' + index + '][total_arrears]'"
                                type="number"
                                class="mt-1 block w-full text-right !leading-nastaliq"
                ...
                <div class="text-right mt-4">
                    <x-primary-button type="submit">Update Sales</x-primary-button>
                </div>
            </form>
        </div>
    </div>
